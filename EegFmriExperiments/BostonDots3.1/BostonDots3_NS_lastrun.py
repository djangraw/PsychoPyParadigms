...
expName = 'BostonDots3_NS'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/molfesepj/Desktop/desktop/BostonDots3/BostonDots3_NS.psyexp',
...
ns.sync()
ns.send_event(key='evt+', timestamp=None, label='Welcome')
...
ns.sync()
ns.send_event(key='evt+', timestamp=None, label='Solid_Block')
# keep track of which components have finished
...
for thisTrial in trials:
    ...
    ns.send_event(key='fix+', timestamp=ms_localtime(), label='fixation')
    ns.send_event(key=str(nstag), timestamp=ms_localtime() + 1000, label=str(nstag))
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *fiximage* updates
        if t >= 0 and fiximage.status == NOT_STARTED:
            # keep track of start time/frame for later
            fiximage.tStart = t
...
ns.sync()
ns.send_event(key='evt+', timestamp=None, label='Striped_Block')
# keep track of which components have finished
...
# update component parameters for each repeat
ns.sync()
ns.send_event(key='evt+', timestamp=None, label='Mixed_Block')
# keep track of which components have finished
