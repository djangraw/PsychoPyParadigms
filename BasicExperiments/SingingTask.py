...
params = {
    ...
    'tStartup': 0,#10.0,      # time after beginning of scan before starting first pre-trial message
    ...
    'msgTime': tPerBeat*4,#6.0,           # duration of pre-trial message (in sec)... will be rounded down to nearest multiple of beat time
    'restTime': tPerBeat*12,#14.0,      # duration of post-trial rest (in sec)... ITI = msgTime+restTime
    'IBI': tPerBeat*12,#14.0,#17,           # time between end of block/probe and beginning of next block (in seconds)
    'nBlocks': 3,             # blocks for this run
    'trialTypes': ['Count','Imagine','Sing'], # currently used only for instructions
    'playPitch': [True, False, True], # Play pitch before each block?
