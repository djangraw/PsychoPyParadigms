...
mov = visual.MovieStim2(win, pathToMovie,
                       size=640,
...
def PlayTappingMovie(pathToMovie, loopDur, blockDur_TRs):
    # Load movie
    mov.loadMovie(pathToMovie)
    ...
    while (nTriggers < blockDur_TRs): # until it's time for the next frame # while mov.status != visual.FINISHED:
        ...
        if mov.getCurrentFrameTime() >= loopDur:
            mov.seek(0) # rewind to beginning
        if mov.status == visual.FINISHED: # odd bug catch
            mov.seek(0)
            shouldflip = mov.play()
...
for iBlock in range(0,params['nBlocks']):
    ...
    print('Tapping Block %d: movie=%s, loopDur=%.2f'%(iBlock, params['movieNames'][iBlock], params['movieDurs'][iBlock]) )
